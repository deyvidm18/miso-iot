...
class RemaView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Inicio de cambios en el codigo para nuevo filtro de ciudad
        # locations = Location.objects.all()
        cityParam = kwargs.get("city", None)
        if cityParam != None:
            city = City.objects.filter(name=cityParam)[0]
            locations = Location.objects.filter(city=city) # filtrar por ciudad
        else:
            locations = Location.objects.all() # regresar todos los locations
        # Fin de cambios en el codigo
